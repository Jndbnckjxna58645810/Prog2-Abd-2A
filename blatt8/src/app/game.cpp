...
unsigned int startGame(std::string const & playerName0, std::string const & playerName1)
{
    ...
    cout << endl << (firstTurn ? playerName1 : playerName0) << " darf beginnen. Tippe [Enter] zum Starten..." << endl;
    getchar();
...
unsigned int getStartingPlayerIdx(string const & playerName0, string const & playerName1)
{
    ...
    cout << "Wir vergleichen nacheinander die jeweils hoechsten Augenzahlen:" << endl
         << left << setw(19) << playerName0 << " " << playerName1 << endl;
...
void selectTypeAndSendMissile(PlayerSea & currentPlayerSea, PlayerSea & otherPlayerSea, unsigned int round)
{
    ...
    switch (round % 3) {
        ...
        default: hit = createVortexOrSendDefaultMissile(currentPlayerSea, otherPlayerSea, round);
    }
    cout << endl << (hit ? "Treffer!" : "Daneben!") << endl << string(80, '=') << endl;
}

bool createVortexOrSendDefaultMissile(PlayerSea & currentPlayerSea, PlayerSea & otherPlayerSea, unsigned int round)
{
    if (round % 2 == 1) { // jedes 2. Mal (alle 6 Runden)
        cout << "Die Peilsender der bisherigen Raketen werden angepeilt..." << endl;
        sleep_for(seconds(3));

        auto trackerCoordinates = currentPlayerSea.collectTrackerCoordinates();
        if (trackerCoordinates.size() >= 3) {

            auto vortex = currentPlayerSea.createVortex(trackerCoordinates, otherPlayerSea);
            cout << currentPlayerSea.getPlayerName() << " erzeugt einen Strudel an der Position " << vortex << endl;
            return vortex.hasHitSomething();
        }
        cout << "Es konnten nicht ausreichend Sender angepeilt werden." << n_endl(2);
    }
    return sendMissile<Missile>(currentPlayerSea, otherPlayerSea);
}
